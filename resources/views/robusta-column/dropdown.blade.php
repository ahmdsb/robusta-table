...
<x-filament::dropdown :max-height="$maxHeight" placement="bottom-end" shift :width="$width"
    ...
    <div x-load
        ...
        x-data="robustaTableColumnManager({
            data: @js($columnsKeys),
            fixed: @js($excludedReorderableColumnsKeys),
            isLoading: false,
        })"
        x-on:sorted="
            isLoading = true;
        ...
        <div x-ref="sortable" class="grid gap-y-4 mt-4 pt-4">
            @foreach ($reorderableColumns as $key => $column)
                ...
                <div x-sortable-item="{{ $column->getName() }}"
                    ...
                    <span class="text-sm font-medium leading-6 text-gray-950 dark:text-white flex-1">
                        {{ $column->getLabel() }}
                    </span>
